...
describe("repayObligationLiquidity", () => {
  ...
  test("repay liquidity mint to reserve", async () => {
    ...
    // pre is lower due to accrued interest
    expect(
      preReserveAcc.liquidity.borrowedAmount.lt(
        postReserveAcc.liquidity.borrowedAmount.add(new BN(repayAmount)),
      ),
    ).toBeTrue();
    // pre is higher due to accrued interest
    expect(
      preReserveAcc.liquidity.availableAmount.gte(
        postReserveAcc.liquidity.availableAmount.sub(new BN(repayAmount)),
      ),
    ).toBeTrue();
    ...
    // pre is lower due to accrued interest
    expect(
      preBorrowerObligationAcc.borrows[0].borrowedAmount.lt(
        postBorrowerObligationAcc.borrows[0].borrowedAmount.add(
          new BN(repayAmount),
        ),
      ),
    ).toBeTrue();
